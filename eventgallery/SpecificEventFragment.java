...
public class SpecificEventFragment extends Fragment {
    ...
    private RecyclerView specific_event_recyclerview;

    public SpecificEventFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_specific_event, container, false);
        parent_view = getActivity().findViewById(android.R.id.content);
        specific_event_recyclerview = (RecyclerView) v.findViewById(R.id.specific_event_recyclerview);
        progressBar = (ProgressBar) v.findViewById(R.id.progBar_eve);
        images = new ArrayList<>();
        mAdapter = new GalleryAdapter(getActivity(), images);

        RecyclerView.LayoutManager mLayoutManager = new GridLayoutManager(getActivity(), 2);
        specific_event_recyclerview.setLayoutManager(mLayoutManager);
        specific_event_recyclerview.setItemAnimator(new DefaultItemAnimator());
        specific_event_recyclerview.addItemDecoration(new SpacingItemDecoration(2, Tools.dpToPx(getContext(), 11), true));
        specific_event_recyclerview.setHasFixedSize(true);

        specific_event_recyclerview.setAdapter(mAdapter);
        specific_event_recyclerview.addOnItemTouchListener(new GalleryAdapter.RecyclerTouchListener(getContext(), specific_event_recyclerview, new GalleryAdapter.ClickListener() {
            @Override
        ...
        loadSpecificEventDetails();
        return v;
    }

    private void loadSpecificEventDetails() {
        ...
        StringRequest newsReq = new StringRequest(Request.Method.POST, Config.GET_STUDENT_BASED_EVENT_URL,
                    ...
                    public void onResponse(String responses)
                    {
                        ...
                        try {
                            ...
                            if (res_value.equals(res_status)) {
                                ...
                                JSONArray studentwisegallerylist = result.getJSONArray("studentwisegallerylist");

                                for (int i = 0; i < studentwisegallerylist.length(); i++) {
                                    try {

                                        JSONObject obj = studentwisegallerylist.getJSONObject(i);
                                ...
                                if(message.equals("No Data...!")){
                                    Toast.makeText(getActivity(), "NO SPECIFIC EVENTS!...", Toast.LENGTH_LONG).show();
                                }else {
