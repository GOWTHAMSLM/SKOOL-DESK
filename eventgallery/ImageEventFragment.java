...
public class ImageEventFragment extends Fragment {

    private String TAG = ImageEventFragment.class.getSimpleName();
    private ArrayList<InnerImage> inner_images;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_image_event, container, false);
        parent_view = v.findViewById(android.R.id.content);

        recyclerView = (RecyclerView) v.findViewById(R.id.inner_recyc_img_view);

        progressBar = (ProgressBar) v.findViewById(R.id.progBar_inner_img_eve);
        inner_images = new ArrayList<>();

        mAdapter = new InnerGalleryAdapter(getActivity(), inner_images);
        ...
        recyclerView.addOnItemTouchListener(new InnerGalleryAdapter.RecyclerTouchListener(getContext(), recyclerView, new InnerGalleryAdapter.ClickListener() {
            ...
            public void onClick(View view, int position) {
                ...
                bundle.putSerializable("images", inner_images);
        ...
        if(!gallery_sys_id.isEmpty()){
            loadInnerImageEventDetails(gallery_sys_id);
        }else{
            //Toast.makeText(getApplicationContext(), "No Data!...", Toast.LENGTH_SHORT).show();
            Snackbar.make(parent_view, "No Image Data!...", Snackbar.LENGTH_SHORT).show();
        }

        return v;
    }

    private void loadInnerImageEventDetails(final String gallerysysid) {
                    ...
                    public void onResponse(String responses)
                    {
                        ...
                        try {
                            ...
                            if (res_value.equals(res_status)) {
                                ...
                                JSONArray galleryurldetails = galleryindividuallist.getJSONArray("galleryurldetails");

                                for (int i = 0; i < galleryurldetails.length(); i++) {
                                    try {

                                        JSONObject obj = galleryurldetails.getJSONObject(i);
                                        InnerImage image = new InnerImage(obj.getString("smallimgurl"), obj.getString("mediumimgurl"), obj.getString("largeimgurl"), obj.getString("originalimgurl"), "image");
                                        // adding movie to movies array
                                        inner_images.add(image);
                                ...
                                if(message.equals("No Data...!")){
                                    Snackbar.make(parent_view, "NO IMAGE GALLERY!...", Snackbar.LENGTH_SHORT).show();
